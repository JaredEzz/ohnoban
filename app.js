...
// A card is picked up by pressing it: a mouse grabs at once, a finger holds
// briefly (HOLD_MS) so a quick swipe still scrolls the board. Once lifted, the
// real <li> is moved between/within column lists for a live preview, a floating
// clone follows the pointer, and the board auto-scrolls near its edges so
// off-screen columns are reachable on a phone. On release the DOM order is
// adopted into state; re-rendering then commits a drop or snaps a cancel back.

let dragged = null;          // the <li> under the pointer
...
let armed = false;           // the card has been picked up; a move now drags it
...
function onPointerMove(e) {
  ...
  if (!armed) {
    // Still waiting on the hold: a real move means the user is scrolling, so
    // drop the gesture and let the browser scroll.
    if (moved > SCROLL_TOL) cancelGesture();
    return;
  }
  if (!dragActive) {
...
function beginDrag(e) {
  dragActive = true;
  dragged.classList.remove("lifted");
...
// Reset everything and detach the window listeners; shared by a cancel and a
// completed drop.
function teardown() {
  ...
  if (dragged) {
    dragged.classList.remove("lifted", "dragging");
    try { dragged.releasePointerCapture(pointerId); } catch {}
  }
  dragged = null; dragOriginColumn = null; armed = false; dragActive = false;
}

// The user started scrolling before the card lifted — abandon the gesture.
function cancelGesture() { teardown(); }
